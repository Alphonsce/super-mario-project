...
class CSVLogger:
    def __init__(self, experiment_name, log_dir="logs"):
        ...
        self.train_fields = [
            "timestamp", "update", "avg_return", "policy_loss", "entropy", "total_loss",
        ]
    ...
    def log_train_step(self, update, avg_return, policy_loss, entropy, total_loss):
        row = {
            "timestamp": datetime.now().isoformat(), "update": update,
            "avg_return": avg_return, "policy_loss": policy_loss,
...
def get_args():
    ...
    parser.add_argument("--experiment", type=str, default="mario_reinforce_default")
...
def train(opt):
    ...
    while True:
        ...
        R = []
        next_return = torch.zeros(opt.num_processes)
        if torch.cuda.is_available():
            next_return = next_return.cuda()
        for reward, done in list(zip(rewards, dones))[::-1]:
            next_return = reward + opt.gamma * next_return * (1 - done)
        ...
        policy_loss = -torch.mean(log_policies * R)
        entropy_loss = torch.mean(entropies)
        total_loss = policy_loss - opt.beta * entropy_loss
        ...
        logger.log_train_step(
            ...
            policy_loss=policy_loss.item(), entropy=entropy_loss.item(),
        ...
        print(f"Update {curr_episode}: avg_return={avg_return:.2f} max_stage={max_stage} "
              ...
              f"pol={policy_loss.item():.4f} ent={entropy_loss.item():.4f}")
