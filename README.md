...
The value loss uses **Smooth L1 (Huber) loss** between the critic's prediction and the GAE-derived target return $\hat{R}_T = \mathcal{A}^{\pi_{old}}(S_T, A_T) + V_\phi(S_T)$:
